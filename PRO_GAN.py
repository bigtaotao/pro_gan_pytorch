...
class ConditionalDiscriminator(th.nn.Module):
    ...
    def __init__(self, num_classes, height=7, feature_size=512, use_eql=True):
        ...
        from torch.nn import ModuleList, AvgPool2d
        from CustomLayers import DisGeneralConvBlock, ConDisFinalBlock
            ...
            for epoch in range(1, epochs[current_depth] + 1):
                ...
                for (i, batch) in enumerate(data, 1):
                    ...
                    if i % int(total_batches / feedback_factor) == 0 or i == 1:
                        ...
                        print("size: %d, Elapsed: [%s]  batch: %d  d_loss: %f  g_loss: %f"
                              % (current_res,melapsed, i, dis_loss, gen_loss))
                        print("sssss")
...
class ConditionalProGAN:
    ...
    def optimize_discriminator(self, noise, real_batch, labels, depth, alpha):
        ...
        for _ in range(self.n_critic):
            ...
            # optimize discriminatorprint(len(next(iter(temp_data_loader))))
            self.dis_optim.zero_grad()
            loss.backward()
            self.dis_optim.step()
    ...
    def train(self, dataset, size,epochs, batch_sizes,
        ...
        make_img_flag = 0
        # create fixed_input for debugging
        temp_data_loader = get_data_loader(dataset, size,batch_sizes[0], num_workers=3)

        cache = next(iter(temp_data_loader))
        fx_labels = cache[0]["label"]
     
        # fuck,fx_labels = next(iter(temp_data_loader))
        # print(fx_labels)
        # time.sleep(5)
        
        label_cache = []
        for each in fx_labels:
            label_cache.append(int(each))
        fx_labels = th.tensor(label_cache)
        ...
        for current_depth in range(start_depth, self.depth):
            ...
            data = get_data_loader(dataset, size,batch_sizes[current_depth], num_workers)
            ticker = 1
            ...
            for epoch in range(1, epochs[current_depth] + 1):
                ...
                for (i, batch) in enumerate(data, 1):
                    ...
                    if i % int(total_batches / feedback_factor) == 0 or i == 1:
                        elapsed = time.time() - global_time
                        elapsed = str(datetime.timedelta(seconds=elapsed))
                        print("size : %d epoch: %d Elapsed: [%s]  batch: %d  d_loss: %f  g_loss: %f "
                              % (current_res,epoch,elapsed, i, dis_loss, gen_loss))
